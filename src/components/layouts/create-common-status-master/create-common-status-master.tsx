...
type CreateCommonStatusMasterProps = {
  sessionId: string;
  onBack: () => void;
};
...
export const CreateCommonStatusMaster = ({
  sessionId,
  onBack,
}: CreateCommonStatusMasterProps): JSX.Element => {
  const [statusCode, setStatusCode] = React.useState<string>("");

  // Single select for Status Name
  ...
  // Fetch Session and extract Modules & Sub-sections
  React.useEffect(() => {
    let isMounted = true;
  ...
  return (
    <Dashboard.Content>
      <Actionbar title="CREATE COMMON STATUS MASTER">
        <Button
          label="SAVE"
      ...
      <Dashboard.Page>
        <Paper>
          ...
          <Paper.Title value="Common Status Details" />

          {/* Row 1: Status Code, Status Name, Modules */}
          <Grid>
            <Grid.Cell size={Grid.CellSize.S4}>
              <TextInput
                ...
                value={statusCode}
                hasError={typeof validation["statusCode"] !== "undefined"}
